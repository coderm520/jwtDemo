...
timeStampStr="1553671875682";
        JwtPayload payload=new JwtPayload("iss",timeStampStr,"subtest","aud","nbf","1516239022","jti",userName);

        JwtModel jwtModel=new JwtModel(header,payload);
        try {
            jwtModel.setSignature(new JwtSignature(getJwtSignatureStr(header,payload)));
            return getFinalToken(jwtModel);
        }
